...
- Gate paid post-checkout features with `offer.entitlementKey`, `src/components/EntitlementGate.tsx`, or the helpers in `src/lib/entitlements.ts`.
- Keep `/payment-success?claim=...` as the Vibiz claim exchange route. It calls the Vibiz runtime claim endpoint, stores the returned entitlement locally, and future visits unlock from the local DB.
