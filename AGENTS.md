...
- Two valid claim-exchange modes (the `/payment-success` page already handles both, do not fork it). **SaaS mode**: Better-Auth is configured (DATABASE_URL set), buyer signs in or signs up, entitlement links to the local `userId` via `storeLocalEntitlement`. Use this when your app has gated dashboards or recurring features. **Funnel mode**: Better-Auth is deconfigured (no DATABASE_URL, `auth` exports `null`), buyer is anonymous, claim is exchanged once to acknowledge the payment, brand-voice thank-you renders, operator delivers the result via email. Use this for one-shot AI funnels (palm readings, business plans, e-books). The `claimVibizEntitlement` helper handles webhook-race retries and refresh idempotency in BOTH modes; do not re-implement the retry loop in app code.
